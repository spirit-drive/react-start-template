...
export const InfoHoverWrapper: FC<infoHoverWrapperProps> = ({ children, hoverComponent }) => {
  ...
  const [elementAbsolutePosition, setElementAbsolutePosition] = useState({
    top: '0',
    right: '0',
    bottom: 'auto',
    left: 'auto',
  ...
  const getElementPositionOnPage = (parentElRef: any) => {
    ...
    if (coordY <= windowVueWidth / 2 && coordX <= windowVueHeight / 2) {
      ...
      setElementAbsolutePosition({ top: 'auto', right: 'auto', bottom: '0', left: '0px' });
    } else {
      setElementAbsolutePosition({ top: 'auto%', right: '0', bottom: '0', left: 'auto' });
    }
  };
  const mouseOn = () => {
