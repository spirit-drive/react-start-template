...
type ColorTypes = [number, number, number];

export const getColorContrastValue = ([red, green, blue] : ColorTypes) =>
...
export const getNumberedArray = <T>(arr : Array<T>) : AnyArrayType<T>[] => arr.map((value, number)  => ({ value, number }));
export const toStringArray = <T>(arr : AnyArrayType<T>[]) : string[] => arr.map(({ value, number }) => `${value}_${number}`);
