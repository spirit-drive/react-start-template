...
export const getNumberedArray = (arr: IArray[]) => arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: IArray[]) => arr.map(({ value, number }) => `${value}_${number}`);
...
interface IAcc {
  [index: string]: IAcc | number | string | boolean;
}

export const transformCustomers = (customers: ICustomer[]) => {
  return customers.reduce((acc: IAcc, customer: ICustomer) => {
