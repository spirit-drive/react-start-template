...
type TContrastValue = 'black' | 'white';
type TError = void | never;
type TColorsValues = [number, number, number];
...
export const getContrastType = (contrastValue: number): TContrastValue => (contrastValue > 125 ? 'black' : 'white');
...
export const hex2rgb = (color: string): TColorsValues => {
  checkColor(color);
...
export const getNumberedArray = (arr: number[]): TNumberedValue[] => arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: TNumberedValue[]): string[] => arr.map(({ value, number }) => `${value}_${number}`);
